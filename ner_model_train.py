...
class RuleFind:
    """Rule-based entity finder using Aho-Corasick algorithm"""
...
def find_entities(tag):
    """Identify all entities and their indices in the tag sequence."""
...
class Nerdataset(Dataset):
    def __init__(
        ...
        self.is_dev = is_dev
        self.entity_extend = EntityExtend()
...
def get_ner_result(model, tokenizer, sen, rule, tfidf_r, device, idx2tag):
    ...
    # print('model results',model_result_word)
    # print('rule results',rule_result)
    tfidf_result = tfidf_r.align(merge_result)
    # print('merged results', merge_result)
    # print('tfidf alignment results', tfidf_result)
    return tfidf_result
...
if __name__ == "__main__":
    ...
    model = BertModel(model_name, hidden_size, len(tag2idx), bi)
