...
def load_model(cache_model):
    ...
    glm_tokenizer = AutoTokenizer.from_pretrained(
        "model/chatglm3-6b-128k", trust_remote_code=True
    )
    glm_model = AutoModel.from_pretrained(
        "model/chatglm3-6b-128k", trust_remote_code=True, device=device
    )
    glm_model.eval()
    # glm_model = None
    # glm_tokenizer = None
...
#  deal with the attribute query of disease, like cause and symptons.
...
# deal with the relation between diseases and other entities like disease and medicine
...
def generate_prompt(
    ...
    # entities is a dict, inlcuding the recognized entities and types
...
def ans_stream(prompt, glm_model, glm_tokenizer):
...
def main(is_admin, usname):
    ...
    with st.sidebar:
        ...
        selected_option = st.selectbox(
            label="请选择大语言模型:", options=["Qwen 1.5", "Llama2-Chinese"]
        )
        choice = (
            "qwen:32b"
            if selected_option == "Qwen 1.5"
            else "llama2-chinese:13b-chat-q8_0"
        )
    ...
    client = py2neo.Graph(
        "http://localhost:7474", user="neo4j", password="neo4j", name="neo4j"
    )
    ...
    if query := st.chat_input(
        ...
        for chunk in ollama.chat(
            model=choice, messages=[{"role": "user", "content": prompt}], stream=True
        ):
